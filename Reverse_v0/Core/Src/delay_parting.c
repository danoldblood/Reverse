...
{
	delay_parting_in(self, input);
	float output = delay_parting_out_modulated_forward(self, modulation);
	delay_parting_tick_forward(self);
	return output;
}
float delay_parting_inOutModulatedReverse(delay_parting *self, float input, float modulation)
{
	delay_parting_in(self, input);
	float output = delay_parting_out_modulated_reverse(self, modulation);
	delay_parting_tick_reverse(self);
	return output;
}
...
float delay_parting_out_modulated_reverse(delay_parting *self, float modulation)
{
	...
	return output * self->xfade * self->volume; //return the output, crossfaded if needed
}
